...
import { GetOrgChartQueryDto, OrgChartScope } from '../dto/organization-structure/chart.dto';
...
export class OrganizationStructureService {
 ...
 after: { status: newStatus },
    ...
    // NEW: hierarchy-based stakeholder notification (REQ-OSM-11)
    if (dto.decision === ApprovalDecision.APPROVED) {
      try {
        await this.notifyStakeholdersByHierarchy(changeRequest, approverEmployeeId.toString());
      } catch (e) {
        console.warn('[NotifyStakeholders] Failed to notify stakeholders:', e);
      }
    }
 ...
 // NEW: REQ-OSM-11 hierarchy-aware stakeholder notifications
 private async notifyStakeholdersByHierarchy(
 changeRequest: StructureChangeRequest,
 approverEmployeeId: string
 ): Promise<void> {
 const stakeholders = new Set<string>();

 // Always include approver
 stakeholders.add(approverEmployeeId);

 // Exclude requester at the end
 const requesterId = changeRequest.requestedByEmployeeId?.toString();

 // Helper to add current holder(s) of a position
 const addPositionHolders = async (positionId?: Types.ObjectId) => {
 if (!positionId) return;
 const holders = await this.assignmentModel
 .find({
 positionId,
 ...
 .lean();
 holders.forEach(h => stakeholders.add(h.employeeProfileId.toString()));
 };

 // Requester's department head (via requester's current assignment)
 try {
 const reqAssignment = await this.assignmentModel
 .findOne({
 employeeProfileId: changeRequest.requestedByEmployeeId,
 $or: [{ endDate: null }, { endDate: { $exists: false } }],
 })
 .sort({ startDate: -1 })
 .lean();

 if (reqAssignment?.departmentId) {
 const reqDept = await this.departmentModel.findById(reqAssignment.departmentId).lean();
 if (reqDept?.headPositionId) {
 await addPositionHolders(reqDept.headPositionId as any);
 }
 }
 } catch (e) {
 console.warn('[NotifyStakeholders] Could not resolve requester department head', e);
 }

 // Department changes → target department head
 if (changeRequest.targetDepartmentId) {
 try {
 const dept = await this.departmentModel.findById(changeRequest.targetDepartmentId).lean();
 if (dept?.headPositionId) {
 await addPositionHolders(dept.headPositionId as any);
 }
 } catch (e) {
 console.warn('[NotifyStakeholders] Could not resolve target department head', e);
 }
 }

 // Position changes → dept head, reporting manager, current holders
 if (changeRequest.targetPositionId) {
 try {
 const pos = await this.positionModel.findById(changeRequest.targetPositionId).lean();
 if (pos) {
 // Dept head of the position's department
 if ((pos as any).departmentId) {
 const dept = await this.departmentModel.findById((pos as any).departmentId).lean();
 if (dept?.headPositionId) {
 await addPositionHolders(dept.headPositionId as any);
 }
 }
 // Reporting manager position holders
 if ((pos as any).reportsToPositionId) {
 await addPositionHolders((pos as any).reportsToPositionId);
 }
 // Current holders of the target position
 await addPositionHolders(pos._id);
 }
 } catch (e) {
 console.warn('[NotifyStakeholders] Could not resolve position hierarchy', e);
 }
 }

 // Remove requester from stakeholder list
 if (requesterId) stakeholders.delete(requesterId);

 const recipients = Array.from(stakeholders);
 console.log(`[NotifyStakeholders] REQ-OSM-11: Notifying ${recipients.length} stakeholders for request ${changeRequest.requestNumber}`);
 console.log(recipients);

 // Batch notify
 await this.sharedOrganizationService.sendStructureChangeNotification(
 'Structural Change Approved',
 `${changeRequest.requestType} (${changeRequest.requestNumber})`,
 recipients
 );
 }
}